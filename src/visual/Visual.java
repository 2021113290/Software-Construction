        ...
        JPanel panel=new JPanel(){
            ...
            public void paint(Graphics g) {
                ...
                for (Object i:c.getTracks()) {
                    ...
                    g.drawOval(ScreenWidth / 2 - ((Track)i).getNumber() * coefficient, ScreenHeight / 2 - ((Track)i).getNumber() * coefficient,
                            ((Track)i).getNumber() * coefficient * 2, ((Track)i).getNumber() * coefficient * 2);
                    if(c.getPhysicalObjects(((Track)i))!=null)
                    for (Object j : c.getPhysicalObjects(((Track)i))) {
                        g.setColor(Color.BLUE);
                        int xOfElectronic = (int) (ScreenWidth / 2
                                + Math.cos(c.getPhysicalObjectPosition(j).getPolarCoordinatePosition().getValue()) * ((Track)i).getNumber()
                                * coefficient);
                        int yOfElectronic = (int) (ScreenHeight / 2
                                + Math.sin(c.getPhysicalObjectPosition(j).getPolarCoordinatePosition().getValue()) * ((Track)i).getNumber()
                                * coefficient);
